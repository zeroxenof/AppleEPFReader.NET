...
using System.IO;
namespace CatchAppAndAppNews.Utils
{
    /// <summary>
    /// Apple EPF Reader
    /// </summary>
    public class AppleEPFReader : IDataReader, IEnumerable<string[]>
    {
        ...
        private string FetchRecord(StreamReader strRd, char recordSeparator)
        {
            ...
            do
            {
                var line = strRd.ReadLine();
                if (line.StartsWith(COMMENT_INITIALIZER.ToString()) && line.EndsWith(recordSeparator.ToString()))
                {
                    if (data.Length > 0)
                    {
                        data.Clear();
            ...
            } while (!strRd.EndOfStream && !data.ToString().EndsWith(recordSeparator.ToString() + Environment.NewLine));
            //Trim(data, recordSeparator);
            return data.ToString().Trim(new char[] { '\n', '\r', recordSeparator });
        }
